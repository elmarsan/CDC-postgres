...
func connect(connParams DBConnParams) *sql.DB {
	connInfo := getConnInfo(connParams)
...
func setupListener(db *sql.DB, event ListenEvent) (*pq.Listener, error) {
	...
	triggerRawQuery := fmt.Sprintf("CREATE TRIGGER %s_notify_event AFTER %s ON %s FOR EACH ROW EXECUTE PROCEDURE cdc_notify_event();", event.Table, event.Event, event.Table)

	_, err = db.Query(triggerRawQuery)
	if err != nil {
	...
	return pq.NewListener(getConnInfo(event.ConnParams), time.Second, time.Second, func(ev pq.ListenerEventType, err error) {
		if err != nil {
...
func getConnInfo(connParams DBConnParams) string {
