...
type Insert struct{ Listen }

func (insert Insert) Listener(event Event) (*pq.Listener, error) {
	...
	_, err = db.Query(
		fmt.Sprintf(`
			...
			CREATE TRIGGER %s_notify_event AFTER INSERT ON %s FOR EACH ROW EXECUTE PROCEDURE cdc_notify_event();
