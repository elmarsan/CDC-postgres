...
type Update struct{ Listen }

func (update Update) Listener(event Event) (*pq.Listener, error) {
	...
	_, err = db.Query(
		fmt.Sprintf(`
			...
			CREATE TRIGGER %s_notify_event AFTER UPDATE ON %s FOR EACH ROW EXECUTE PROCEDURE cdc_notify_event();
